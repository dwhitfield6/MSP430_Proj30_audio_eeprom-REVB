...
*/
    ...
    void main(void)
    {
       ...
       for(i=0; i<255;i++)
       {
       i2c_tx(EEPROM_ADDR, &count, 1,TWOBYTEADDR,0x00,count);//i2c TX 115 bytes starting @ address 01:00
       count++;
       }
       ...
       i2c_rx(EEPROM_ADDR, rxdata, 2,TWOBYTEADDR,0x00,0x00);//i2c RX 115 bytes from EEPROM starting @ address 01:00
